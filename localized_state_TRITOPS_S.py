...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 10:46:42 2023
...
eigenvalues = data["eigenvalues_sparse"]
eigenvectors = data["eigenvectors_sparse"]
...
ax.set_xlabel(r"$\ell$")
ax.set_ylabel("Particle density at the junction")
...
for i in [0,1]:
    ...
    destruction_up, destruction_down, creation_down, creation_up = get_components(state, 2*params["L_x"], params["L_y"])
    ...
    localized_state = zero_state[i][:, 299, :]

fig, ax = plt.subplots(2,2)
ax[0,0].set_title("Creation up")
ax[0,0].plot(np.real(localized_state[:,0]), label="Real part")
ax[0,0].plot(np.imag(localized_state[:,0]), label="Imaginary part")
ax[0,1].set_title("Creation down")
ax[0,1].plot(np.real(localized_state[:,1]), label="Real part")
ax[0,1].plot(np.imag(localized_state[:,1]), label="Imaginary part")
ax[1,0].set_title("Destruction down")
ax[1,0].plot(np.real(localized_state[:,2]), label="Real part")
ax[1,0].plot(np.imag(localized_state[:,2]), label="Imaginary part")
ax[1,1].set_title("Destruction up")
ax[1,1].plot(np.real(localized_state[:,2]), label="Real part")
ax[1,1].plot(np.imag(localized_state[:,2]), label="Imaginary part")
...
def psi_1_prime(y, kappa, m_0, Delta, L):
    ...
    C_L = np.sqrt(kappa/(2*(alpha*(alpha+1)-np.exp(-2*kappa*L)*(alpha+(alpha+1)*kappa*L))))
    if y<=0:
...
ax.plot(y, [psi_1_prime(y_value, kappa, m_0, Delta, L=100) for y_value in y])