...
class ZKMSuperconductivity():
    ...
    .. math ::
       ...
       H = \frac{1}{2} \sum_n^{L_x} \sum_m^{L_y} \vec{c}^\dagger_{n,m} 
       \left(-\mu 
          \tau_z\sigma_0 +\Delta_0\tau_x\sigma_0 \right) \vec{c}_{n,m}
       + \frac{1}{2}
       \sum_n^{L_x-1}\sum_m^{L_y}\left[\mathbf{c}^\dagger_{n,m}\left(
           -t\tau_z\sigma_0 +
...
class ZKMSparseSuperconductor(ZKMSuperconductivity,
                                  SparseHamiltonian):
    def __init__(self, L_x:int, L_y: int, t:float, mu:float,
                 Delta_0:float, Delta_1:float, Lambda:float):
        ZKMSuperconductivity.__init__(self, t, mu, Delta_0, Delta_1, Lambda)
        SparseHamiltonian.__init__(self, L_x, L_y, self._get_onsite(), 
                             self._get_hopping_x(),
                            self._get_hopping_y())
        
class ZKMSparseSuperconductorPeriodicInY(ZKMSuperconductivity,
                                       SparsePeriodicHamiltonianInY):
    def __init__(self, L_x:int, L_y:int, t:float, mu:float,
                 Delta_0:float, Delta_1:float, Lambda:float):
        ZKMSuperconductivity.__init__(self, t, mu, Delta_0, Delta_1, Lambda)
...
class ZKMSuperconductorKY(ZKMSuperconductivity, Hamiltonian):
    r"""ZKM superconductor for a given k in the y direction.
    
    .. math::

        H_{ZKM} = \frac{1}{2}\sum_k H_{ZKM,k}
